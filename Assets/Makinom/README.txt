...
Makinom 1 has been replaced by Makinom 2 and will be deprecated in the future.
Owners of Makinom Pro can upgrade to Makinom 2 Pro for free once it’s available in the Asset Store, 
and there’s also a Makinom 2 Free version available.

If you want to upgrade your Makinom 1 project, check out the Makinom Upgrade Tool:
...
The full source code is included - see the 'Package Description' section for details.
...
Please search for a solution in the 'Support & Community' forum:
...
- Makinom.unitypackage
3 DLLs containing Makinom.
Gizmo icons in 48x48 pixels.

- gameplay_source_code.zip ('Pro' version only)
Contains the full source code in a Visual Studio project.
...
Version 1.19.1:
- new: Source Code: Makinom now contains the full C# source code ('Pro' version only).
- fix: Float Fields: Fixed an issue where 'Formula' value types didn't display the formula selection.
- fix: Float Fields: Fixed an issue where 'Input Key Axis' value types didn't display the input key selection.


Version 1.19.0:
- change: Schematics: Raycast: 'Layer Mask' and 'Origin Offset' are now advanced value selection fields. Old settings will be updated automatically.
- change: Cameras: If no camera is tagged as 'Main Camera', Makinom will use the first available camera in the scene (via 'Camera.allCameras'). Makinom needs to know about a camera to correctly position world position based GUI elements, e.g. flying texts.
- fix: Scene Objects: Fixed an issue where cursor changes (and tooltips) where not changed when the scene object becomes unavailable (e.g. due to variable conditions) while the cursor is over the game object.
- fix: Scene Changes: Changing scenes (e.g. using 'Scene Changer') didn't continue fading in and spawning the player when defining the scene name with the folder the scene was in (e.g. 'Folder/SceneName' instead of just 'SceneName').
- fix: Formulas: Using 'Use Minimum Value' and 'Use Maximum Value' at the same time lead to only using the minimum value check.


Version 1.18.0:
- new: Float Fields: 'Vector3 Distance' selection available in all float field type selections. Uses the distance between 2 Vector3 values.
...
- new: Schematics: Store Predicted Position: 'Store Predicted Position' node available in 'Movement > Movement' nodes. Store the future position of a position (e.g. from a game object) based on a defined velocity into a Vector3 variable.
- change: Flying Texts: Flying texts are now scaled using the default GUI scaling ('Menus > Menu Settings') to keep the correct aspect ratio.
- fix: Schematics: Check Transform: Fixed an issue where the 'Check Transform' node's settings messed up the editor layout.
...
- change: New UI: When not using a texture mask, the new 'Rect Mask 2D' component will be used for masking content. This reduces draw calls and increases performance.
